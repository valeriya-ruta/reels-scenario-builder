...
1. ВІДКРИТТЯ (слайди 1–3) — тільки інтрига:
   - Не розкривай суть у перших 3 слайдах. Змушуй гортати далі.
   - Слайд 1: ЗАВЖДИ visual = "Говоряча голова" або "Відео в тему". Ніколи не фото і не кольоровий фон для першого слайду.
   - Один зі слайдів 2–3: ОБОВ'ЯЗКОВО interactive = "Стікер". Це найлегша взаємодія — не питання, не опитування, саме стікер.
...
4. ВИБІР ІНТЕРАКТИВУ (interactive):
...
Мінімум 5, максимум 9.
Кожен слайд = одна думка. Не намагайся заповнити до 9 — краще 6 чітких, ніж 9 розмитих.
  ...
  "slides": [
    {
      ...
      "voiceover": "Це не про лінь. Це про те, як я сама собі ускладнила все.",
    ...
    },
    {
      ...
      "voiceover": "Але щось всередині не давало зробити простий крок. Знайоме?",
      "visual": "Говоряча голова",
      "interactive": "Стікер",
...
function normalizeOutput(raw: unknown): StoriesOutput {
  ...
  const slides: Slide[] = slidesSeed.map((slideLike, index) => {
    ...
    const screenText = typeof slideObj.screen_text === 'string' && slideObj.screen_text.trim()
      ...
      : 'Текст на екрані';
    const voiceover = typeof slideObj.voiceover === 'string' && slideObj.voiceover.trim()
      ? slideObj.voiceover.trim()
      : 'Озвучення для цієї сторіс.';

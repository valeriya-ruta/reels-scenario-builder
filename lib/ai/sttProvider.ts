...
// OpenRouter's /audio/transcriptions accepts multipart uploads up to 25MB, the
// same ceiling the Groq Whisper path enforced.
...
async function parseTranscriptionResponse(sttRes: Response): Promise<TranscriptResult> {
  const parsed = (await sttRes.json()) as WhisperTranscriptionResponse;
...
const WHISPER_AUDIO_MAX_BYTES = WHISPER_MAX_UPLOAD_BYTES;

/**
 * Transcribes raw audio bytes uploaded directly from the browser (MediaRecorder
 * capture) through OpenRouter's OpenAI-compatible Whisper endpoint — same model
 * (`whisper-large-v3-turbo`), same direct-bytes multipart upload as reel
 * transcription. No ffmpeg, no remote fetch. Used by the braindump voice capture.
 */
export async function transcribeAudioFile(
  ...
  if (size > WHISPER_AUDIO_MAX_BYTES) {
  ...
  const sttRes = await openRouterTranscribe({ file: upload, language: options.language });
  if (!sttRes.ok) {
...
/**
 * Transcribes audio from a remote media file. Intentionally does **not** pass `url`
 * to the transcription API: hosted fetch often fails on Instagram/TikTok CDNs, so
 * the app downloads the bytes server-side and uploads them directly.
 */
export async function transcribeMediaFromUrl(mediaUrl: string): Promise<TranscriptResult> {
  ...
  if (mediaBuffer.byteLength > WHISPER_MAX_UPLOAD_BYTES) {
    throw new Error(
  ...
  const sttRes = await openRouterTranscribe({ file: upload });
